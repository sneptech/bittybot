...
must_haves:
  ...
  key_links:
    - from: "lib/features/chat/application/chat_notifier.dart"
      ...
      via: "ref.read(modelReadyProvider).value to get LlmService"
...
- Get LlmService, call `stop(state.activeRequestId!)`
...
4. Get LlmService: `ref.read(modelReadyProvider).value!`
5. Subscribe to `llmService.responseStream` (if not already subscribed)
6. Call `llmService.generate(prompt: prompt, nPredict: 512)` — 512 tokens for chat per locked decision
...
Before each `_processMessage`, estimate token usage with `PromptBuilder.estimateTokenCount()` on the accumulated prompt text. If approaching ~90% of nCtx (2048), set a flag in state indicating context is nearly full. The UI (Phase 6) will use this to show a "Start new session" prompt. Do NOT auto-start a new session — just signal the state.
...
4. Get LlmService, call `generate(prompt: prompt, nPredict: 128)` — 128 tokens for translation per locked decision
